...
function getFirstPromiseResult(promises) {
  return Promise.race(promises.map((promise) => Promise.resolve(promise)));
}
...
function getAllResult(promises) {
  return Promise.allSettled(promises).then((promisesNames) => {
    return promisesNames.map((promise) =>
      promise.reason === 'rejected' ? null : promise.value
    );
  });
}
